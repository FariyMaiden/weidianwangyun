...
<!-- 实时监测-》用电查询=>月查询用电 -->
    <div class="bigbox">
          <div>
        <el-button @click="ribtn">日用电查询</el-button>
        
        </div>
        <div class="style1">

            用户类别：
            <el-select v-model="tiaojianchaxun.meterType" placeholder="请选择用户类别">
                <el-option
           ...
           用户名称：
            ...
            placeholder="请输入用户名称" 
            v-model="tiaojianchaxun.countPoint"
            ></el-input>
...
<script>
    export default {
        ...
        methods: {
         ...
         //跳转到日用电查询上
            ribtn(){
                this.$router.push('/dianbiaoshuju')
            },
             // 递归方法
            getTreeData(data){
                // 循环遍历json数据
        ...
        this.searchshuzi(val);
            ...
            searchshuzi(val){   
                ...
                this.$axios({
                    method:'GET',
                    url:this.api.baseUrl + this.api.electricitymonth,
                    params:params,
                    type:'json'
                }).then(response=>{
                   console.log(response)
